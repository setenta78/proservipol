...
// Obtener la URL actual (solo el nombre del script)
...
<aside class="fixed top-16 w-64 h-full bg-gray-500 text-white">
    <ul class="space-y-1 p-4 text-sm">
        ...
        <li>
            <?php
            // Ahora el Gestor vive en /app/gestor-usuarios/gestor_usuarios.php
            $gu_target   = '/app/gestor-usuarios/gestor_usuarios.php';
            $gu_basename = basename($gu_target); // gestor_usuarios.php
            ?>
            <a href="<?php echo $gu_target; ?>"
               class="flex items-center space-x-2 py-2 px-4 rounded <?php echo ($current_page == $gu_basename) ? 'bg-green-700 text-white' : 'hover:bg-gray-700'; ?>">
        ...
        <li>
            <?php
            $bi_target   = '/app/buscar-ingresos/buscar_ingresos_usuario.php';
            $bi_basename = basename($bi_target); // buscar_ingresos_usuario.php
            ?>
            <a href="<?php echo $bi_target; ?>"
               class="flex items-center space-x-2 py-2 px-4 rounded <?php echo ($current_page == $bi_basename) ? 'bg-green-700 text-white' : 'hover:bg-gray-700'; ?>">
        ...
        <li>
            <?php
            // Dentro de /app/camaras-corporales/ mantenemos la lógica PHP 5.1.2
            $cam_target = (version_compare(phpversion(), '5.1.2', '=='))
                ? '/app/camaras-corporales/camaras.php'
                : '/app/camaras-corporales/camaras_local.php';

            $cam_basename = basename($cam_target); // camaras.php o camaras_local.php
            ?>
            <a href="<?php echo $cam_target; ?>"
               class="flex items-center space-x-2 py-2 px-4 rounded <?php echo ($current_page == $cam_basename) ? 'bg-green-700 text-white' : 'hover:bg-gray-700'; ?>">
